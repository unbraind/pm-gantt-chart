...
// 2026.6.2 enhancement. The documented set is milestone|type|assignee.
type GroupBy = "milestone" | "tag" | "type" | "assignee";
...
function buildRows(items: PmItem[], opts: GanttOptions, windowStart: Date): GanttRow[] {
  const criticalIds = opts.criticalPath ? computeCriticalPath(items) : new Set<string>();
...
function resolveGanttOptions(options: Record<string, unknown>): ResolvedOptions {
  ...
  const groupBy: GroupBy = ["milestone", "tag", "type", "assignee"].includes(String(rawGroupBy))
...
type ExportFormat = "mermaid" | "html" | "ascii";
  ...
  activate(api: ExtensionApi) {
    api.registerCommand({
      ...
      flags: [
        ...
        },
        {
          ...
          value_name: "field",
          description:
            "Group items by: milestone (sprint/release) | type | assignee | tag (default: milestone)",
        },
        {
          long: "--status",
          value_name: "filter",
    ...
    api.registerExporter("gantt", async (ctx: ImportExportContext) => {
      ...
      if (!["mermaid", "html", "ascii"].includes(rawFormat)) {
        throw new CommandError(
          `Unknown --format "${rawFormat}". Valid: mermaid | html | ascii.`,
          EXIT_CODE.USAGE,
        );
      }
